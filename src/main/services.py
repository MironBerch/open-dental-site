...
def get_search_results(query: str):
    ...
    # Staff results
    staff_results = (
        Staff.objects.filter(published=True)
        .filter(Q(fio__iregex=query) | Q(information__iregex=query))
        .values('id', 'fio', 'information', 'slug')
    )

    for staff in staff_results:
        match_count = staff['fio'].lower().count(query.lower()) + staff[
            'information'
        ].lower().count(query.lower())
    ...
    for group in service_group_results:
        match_count = group['name'].lower().count(query.lower()) + group[
            'description'
        ].lower().count(query.lower())
    ...
    service_results = (
        ...
        .values(
            'id', 'name', 'information', 'short_information', 'slug', 'group__name', 'group__slug'
        )
    )

    for service in service_results:
        match_count = (
            service['name'].lower().count(query.lower())
    ...
    # Works results
    work_results = (
        Work.objects.filter(published=True)
        .filter(Q(name__iregex=query) | Q(information__iregex=query))
        .values('id', 'name', 'information', 'slug')
    )

    for work in work_results:
        match_count = work['name'].lower().count(query.lower()) + work['information'].lower().count(
            query.lower()
        )
